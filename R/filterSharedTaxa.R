...
#' @param input1_format To load from a local file: 'nexus', 'fasta', or 'tnt'. Defaults to NULL (auto-detects).
...
#' @param input2_format To load from a local file: 'nexus', 'fasta', or 'tnt'. Defaults to NULL (auto-detects).
...
#' )
#' @export
filterSharedTaxa <- function(input1, input1_format = NULL, input2, input2_format = NULL,
                             output_path = NULL, output_format = NULL, output_concatenate = TRUE,
                             return_as_list = FALSE, level = "strictly_shared") {
  ...
  detect_format_from_nexus <- function(nexus_file) {
    ...
    if (length(format_line) > 0) {
      if (grepl("datatype\\s*=\\s*protein", format_line[1]) || grepl("protein", format_line[1])) {
        ...
        return("dna")
      } else if (grepl("datatype\\s*=\\s*rna", format_line[1]) || grepl("rna", format_line[1])) {
        return("dna")
      } else if (grepl("datatype\\s*=\\s*standard", format_line[1]) || grepl("standard", format_line[1])) {
        return("standard")
      }
    }
    return("unknown")
  ...
  # Helper function to detect data type from TNT file headers
  ...
  # Helper function to auto-detect format from the matrix content directly
  detect_format_from_matrix <- function(mat) {
    mat_upper <- toupper(as.vector(mat))

    # Remove brackets, parentheses, curly braces for parsing tokens
    mat_upper <- gsub("[\\[\\]\\(\\)\\{\\}]", "", mat_upper)

    # Split multi-character elements into individual characters
    chars <- unlist(strsplit(mat_upper, ""))

    # Ignore missing and gap characters
    valid_chars <- chars[!(chars %in% c("?", "-", "N", "X", ""))]
    if (length(valid_chars) == 0) {
      return("standard")
    }

    # Check for numbers, which strongly indicates morphological/standard data
    if (any(grepl("[0-9]", valid_chars))) {
      return("standard")
    }

    # Protein unique amino acids (exclude DNA ambiguous characters)
    protein_only <- c("E", "F", "I", "L", "P", "Q", "Z", "J", "O")
    if (any(valid_chars %in% protein_only)) {
      return("protein")
    }

    # Check if all characters are valid DNA/RNA bases (including ambiguity codes)
    dna_codes <- c("A", "C", "G", "T", "U", "R", "Y", "S", "W", "K", "M", "B", "D", "H", "V")
    if (all(valid_chars %in% dna_codes)) {
      return("dna")
    }

    # If not completely DNA, check if it's predominantly letters (protein fallback)
  ...
  align_matrix_to_taxa <- function(mat, taxa) {
    aligned <- matrix("?", nrow = length(taxa), ncol = ncol(mat), dimnames = list(taxa, colnames(mat)))
  ...
  validate_output_files <- function(filenames) {
    ...
    if (length(missing_dirs) > 0) {
      msg <- paste0("Output directory does not exist: ", paste(missing_dirs, collapse = ", "))
    ...
    if (length(unwritable_dirs) > 0) {
      stop("Output directory is not writable: ", paste(unwritable_dirs, collapse = ", "), call. = FALSE)
    }
  }
  ...
  # Helper function to write TNT format with multiple blocks
  ...
  if (is.matrix(input1) || is.data.frame(input1)) {
    ...
    if (is.null(input1_format)) {
      if (is.character(input1)) {
        if (grepl("\\.tnt$", input1, ignore.case = TRUE)) {
          input1_format <- "tnt"
        } else if (grepl("\\.nex(us)?$", input1, ignore.case = TRUE)) {
          input1_format <- "nexus"
        } else if (grepl("\\.fa(s|sta)?$", input1, ignore.case = TRUE)) {
          input1_format <- "fasta"
        } else {
          stop("Could not auto-detect input1_format from file extension. Please provide input1_format = 'nexus', 'fasta', or 'tnt'")
        }
      } else {
        stop("input1 must be a matrix, data frame, or a file path.")
      }
    }
  ...
  if (format1 == "unknown") {
    format1 <- detect_format_from_matrix(mat1)
  }
  ...
  if (is.matrix(input2) || is.data.frame(input2)) {
    ...
    if (is.null(input2_format)) {
      if (is.character(input2)) {
        if (grepl("\\.tnt$", input2, ignore.case = TRUE)) {
          input2_format <- "tnt"
        } else if (grepl("\\.nex(us)?$", input2, ignore.case = TRUE)) {
          input2_format <- "nexus"
        } else if (grepl("\\.fa(s|sta)?$", input2, ignore.case = TRUE)) {
          input2_format <- "fasta"
        } else {
          stop("Could not auto-detect input2_format from file extension. Please provide input2_format = 'nexus', 'fasta', or 'tnt'")
        }
      } else {
        stop("input2 must be a matrix, data frame, or a file path.")
      }
    }
  ...
  if (format2 == "unknown") {
    format2 <- detect_format_from_matrix(mat2)
  }
  ...
  cat("Format matrix 1:", format1, "\n")
  cat("Format matrix 2:", format2, "\n")
